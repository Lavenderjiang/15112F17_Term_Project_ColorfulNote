...
def debugPlot():
    ...
    while first.stop == False:
        ...
        final_y = np.abs(y_fft[:CHUNK]) * 2 / (128*CHUNK)
        line_fft.set_ydata(final_y)
        
        freqs =line_fft.get_xdata()
        maxAmp = max(final_y[20:])
        #index of the max frequency
        maxi = np.where(final_y[20:]==maxAmp)
        curFreq= freqs[maxi]+157
        
        #make sure curFreq is in legal range
        if curFreq<800:
            res.append(curFreq)
        else:
            res.append(1600-curFreq)
